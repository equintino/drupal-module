...
namespace Drupal\booklet\Plugin\Field\FieldFormatter;
...
/**
 * Plugin implementation of the 'booklet_default' formatter.
 *
 * @FieldFormatter(
 *   id = "booklet_formatter_default",
 *   label = @Translation("Booklet Map"),
 *   field_types = {
 *     "geofield"
 *   }
 * )
 */
class BookletDefaultFormatter extends FormatterBase implements ContainerFactoryPluginInterface {
  ...
  /**
   * BookletDefaultFormatter constructor.
   *
  ...
  public function settingsSummary() {
    ...
    $summary = [];
    $summary[] = $this->t('Booklet Map: @map', ['@map' => $settings['booklet_map']]);
  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
    ...
    $map = booklet_map_get_info($settings['booklet_map']) ?? $default_settings['booklet_map'];

    // Add a specific map id.
    $map['id'] = Html::getUniqueId("booklet_map_{$entity_type}_{$bundle}_{$entity_id}_{$field->getName()}");
    ...
    foreach ($items as $item) {
      ...
      $points = $this->bookletService->bookletProcessGeofield($item->value);
      $feature = $points[0];
      $feature['entity_id'] = $entity_id;

      // Attach tooltip data (value & options).
      if (isset($settings['booklet_tooltip']) && !empty($settings['booklet_tooltip']['value'])) {
        $feature['tooltip'] = $settings['booklet_tooltip'];
        // Decode any entities because JS will encode them again,
        // and we don't want double encoding.
        $feature['tooltip']['value'] = $this->tokenResolvedContent($entity, $settings['booklet_tooltip']['value'], $tokens, $results);

        // Associate dynamic tooltip options (token based).
        if (!empty($settings['booklet_tooltip']['options'])) {
          $feature['tooltip']['options'] = $this->tokenResolvedContent($entity, $settings['booklet_tooltip']['options'], $tokens, $results);
        }
      }
      ...
      if ($popup_control) {
        $feature['popup'] = $settings['booklet_popup'];
        ...
        if (!empty($settings['booklet_popup']['options'])) {
          $feature['popup']['options'] = $this->tokenResolvedContent($entity, $settings['booklet_popup']['options'], $tokens, $results);
        }
      }

      // Add/merge eventual map icon definition from hook_booklet_map_info.
      if (!empty($map['icon'])) {
        $settings['icon'] = $settings['icon'] ?: [];
        // Remove empty icon options so thxat they might be replaced by the
        // ones set by the hook_booklet_map_info.
      ...
      $this->moduleHandler->alter('booklet_formatter_feature', $feature, $item, $entity);
      $features[] = $feature;
    }
    ...
    $this->moduleHandler->alter('booklet_default_map_formatter', $js_settings, $items);
    ...
    if (!empty($settings['multiple_map'])) {
      foreach ($js_settings['features'] as $k => $feature) {
        $map = $js_settings['map'];
        $map['id'] = $map['id'] . "-{$k}";
        $results[] = $this->bookletService->bookletRenderMap($map, [$feature], $map_height);
      }
    }
    ...
    elseif (!empty($js_settings['features']) || empty($settings['hide_empty_map'])) {
      $results[] = $this->bookletService->bookletRenderMap($js_settings['map'], $js_settings['features'], $map_height);
    }
