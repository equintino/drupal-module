...
namespace Drupal\booklet\Plugin\Field\FieldWidget;
...
/**
 * Plugin implementation of the "booklet_widget" widget.
 *
 * @FieldWidget(
 *   id = "booklet_widget_default",
 *   label = @Translation("booklet Map (default)"),
 *   description = @Translation("Provides a booklet Widget with Geoman Js Library."),
 *   field_types = {
 *     "geofield",
 *   },
 * )
 */
class BookletDefaultWidget extends GeofieldDefaultWidget {
  ...
  /**
   * Get maps available for use with booklet.
   */
  protected static function getBookletMaps() {
    $options = [];
  ...
  public static function defaultSettings() {
    $base_layers = self::getBookletMaps();

    return array_merge(parent::defaultSettings(), [
      'map' => [
        'booklet_map' => array_shift($base_layers),
        'height' => 400,
        'auto_center' => TRUE,
  ...
  public function settingsForm(array $form, FormStateInterface $form_state) {
    ...
    $form['map']['booklet_map'] = [
      '#title' => $this->t('booklet Map'),
      '#type' => 'select',
      '#options' => ['' => $this->t('-- Empty --')] + $this->getBookletMaps(),
      '#default_value' => $map_settings['booklet_map'] ?? $default_settings['map']['booklet_map'],
      '#required' => TRUE,
    ];
    $form['map']['height'] = [
      '#title' => $this->t('Height'),
    ...
    $form['toolbar'] = [
      '#type' => 'fieldset',
      '#title' => $this->t('booklet PM Settings'),
    ];
  ...
  public function formElement(
    ...
    $map = booklet_map_get_info($map_settings['booklet_map'] ?? $default_settings['map']['booklet_map']);

    // Add a specific map id.
    $map['id'] = Html::getUniqueId("booklet_map_widget_{$entity_type}_{$bundle}_{$entity_id}_{$field->getName()}");
    ...
    // for backward compatibility with booklet release < 2.x.
    if (!empty($map_settings["locate"])) {
      $previous_automatic_locate_settings = TRUE;
    }

    // Extend map settings to additional options
    // to uniform with booklet Formatter and booklet View processing.
    ...
    $json_element_name = 'booklet-widget-input';
    ...
    $this->moduleHandler->alter('booklet_default_widget', $map, $this);

    $element['map'] = $this->bookletService->bookletRenderMap($map, [], $map_settings['height'] . 'px');
    ...
    // Build JS settings for the booklet Widget.
    $booklet_widget_js_settings = [
    ...
    // booklet.widget plugin.
    $element['map']['#attached']['library'][] = 'booklet/booklet-widget';

    // Settings and geo-data are passed to the widget keyed by field id.
    $element['map']['#attached']['drupalSettings']['booklet'][$element['map']['#map_id']]['booklet_widget'] = $booklet_widget_js_settings;
