...
/**
 * Class bookletSettingsElementsTrait.
 *
 * Provide common functions for booklet Settings Elements.
 *
 * @package Drupal\booklet
 */
trait BookletSettingsElementsTrait {

  /**
   * booklet Controls Positions Options.
   *
   * @var array
   */
  ...
  /**
   * booklet Circle Radius Marker Field Types Options.
   *
   * @var array
   */
  protected $bookletCircleRadiusFieldTypesOptions = [
    'integer',
    'float',
    'decimal',
  ];
  ...
  public static function getDefaultSettings() {
    return [
      ...
      // @todo Keep this for backword compatibility with booklet < 2.x.
      'popup' => FALSE,
      // @todo Keep this for backword compatibility with booklet < 2.x.
      'popup_content' => '',
      'booklet_popup' => [
      ...
      'icon' => [
        ...
        'html' => '<div></div>',
        'html_class' => 'booklet-map-divicon',
        'circle_marker_options' => '{"radius": 100, "color": "red", "fillColor": "#f03", "fillOpacity": 0.5}',
      ],
      'booklet_markercluster' => [
        'control' => FALSE,
  ...
  /**
   * Generate the booklet Map General Settings.
   *
  ...
  protected function generateMapGeneralSettings(array &$elements, array $settings) {
    ...
    $booklet_map_options = [];
    foreach (booklet_map_get_info() as $key => $map) {
      $booklet_map_options[$key] = $map['label'];
    }

    $booklet_map = $settings['booklet_map'] ?? $settings['map'];

    $elements['booklet_map'] = [
      '#title' => $this->t('booklet Map Tiles Layer'),
      '#description' => $this->t('Choose the @booklet_map_tiles Layer to start the Map with (@see hook_booklet_map_info).', [
        '@booklet_map_tiles' => $this->link->generate("booklet Js Library Map Tiles", Url::fromUri("https://leafletjs.com/reference.html#tilelayer", ['attributes' => ['target' => 'blank']])),
      ]),
      '#type' => 'select',
      '#options' => $booklet_map_options,
      '#default_value' => $booklet_map,
      '#required' => TRUE,
    ];
    ...
    $elements['height_unit'] = [
      ...
      '#description' => t("Whether height is absolute (pixels) or relative (percent, vertical height).<br><strong>Note:</strong> In case of Percent the booklet Map should be wrapped in a container element with defined Height, otherwise won't show up."),
    ];
    ...
    $elements['gesture_handling'] = [
      ...
      '#description' => $this->t('Enable the @gesture_handling_link functionality for the Map.', [
        '@gesture_handling_link' => $this->link->generate($this->t('booklet Gesture Handling Library'), Url::fromUri('https://github.com/elmarquis/Leaflet.GestureHandling', [
          'absolute' => TRUE,
  ...
  protected function setFitBoundsOptionsElement(array &$element, array $settings) {
    ...
    $fitbounds_options_description = $this->t('Set here options that will be applied when fitBounds is triggered (e.g. Zoom, Pan, Padding).<br>Refer to the @fitbounds_options_documentation.', [
      '@fitbounds_options_documentation' => $this->link->generate($this->t('booklet Fitbound Options Documentation'), Url::fromUri('https://leafletjs.com/reference.html#fitbounds-options', [
        'absolute' => TRUE,
        'attributes' => ['target' => 'blank'],
      ])),
    ]);
  ...
  /**
   * Generate the booklet Map Position Form Element.
   *
   * @param array $map_position_options
   *   The map position options array definition.
   *
   * @return array
   *   The booklet Map Position Form Element.
   */
...
<b>Note:</b> It is possible to remove the Zoom Control making Min and Max Zoom coincident (the initial Zoom value will be forced to those).'),
    ...
    $element['zoom'] = [
      ...
      '#min' => 0,
      '#max' => 22,
      '#description' => $this->t('The initial Zoom level for the booklet Map (when empty or when Forced).<br>Admitted values usually range from 0 (the whole world) to 20 - 22, depending on the max zoom supported by the specific Map Tile in use.<br>As a reference consider Zoom 5 for a large country, 10 for a city, 15 for a road or a district, etc.'),
   ...
   *
   * @return array
   *   The booklet weight Form Element.
   */
  ...
  protected function generateIconFormElement(array $icon_options) {
    ...
    $icon_url_description = $this->t('Can be an absolute or relative URL (as Drupal root folder relative paths <strong>without the leading slash</strong>) <br><b>If left empty the default booklet Marker will be used.</b><br>@token_replacement_disclaimer', [
    ...
    $element = [
      ...
      'description' => [
        '#markup' => $this->t('For details on the following setup refer to @booklet_icon_documentation_link', [
          '@booklet_icon_documentation_link' => $this->bookletService->bookletIconDocumentationLink(),
        ]),
      ],
    ];
    ...
    $element['html'] = [
      '#title' => $this->t('Html'),
      '#type' => 'textarea',
      '#description' => $this->t('Insert here the Html code that will be used as marker html markup. <b>If left empty the default booklet Marker will be used.</b><br>@token_replacement_disclaimer', [
        '@token_replacement_disclaimer' => $token_replacement_disclaimer,
      ]),
    ...
    $element['circle_marker_options'] = [
      ...
      '#description' => $this->t('An object literal of Circle Marker options, that comply with the @booklet_circle_marker_object.<br>The syntax should respect the javascript object notation (json) format.<br>As suggested in the field placeholder, always use double quotes (") both for the indexes and the string values.<br><b>Note: </b> Use <strong>Replacement Patterns</strong> to input dynamic values.<br>Empty value will fall back to default booklet Circle Marker style.', [
        '@booklet_circle_marker_object' => $this->link->generate('booklet Circle Marker object', Url::fromUri('https://leafletjs.com/reference.html#circlemarker', [
          'absolute' => TRUE,
          'attributes' => ['target' => 'blank'],
        ])
        ),
    ...
    if (method_exists($this, 'getProvider') && $this->getProvider() == 'booklet_views') {
  ...
  protected function setMapPathOptionsElement(array &$element, array $settings) {
    ...
    $path_description = $this->t('Set here options that will be applied to the rendering of Map Path Geometries (Lines & Polylines, Polygons, Multipolygons, etc.).<br>Refer to the @polygons_documentation.<br>Note: If empty the default booklet path style, or the one choosen and defined in booklet.api/hook_booklet_map_info, will be used.<br>@token_replacement_disclaimer', [
      '@polygons_documentation' => $this->link->generate($this->t('booklet Path Documentation'), Url::fromUri('https://leafletjs.com/reference.html#path', [
        'absolute' => TRUE,
        'attributes' => ['target' => 'blank'],
      ])),
      '@token_replacement_disclaimer' => $token_replacement_disclaimer,
  ...
  protected function setAdditionalMapOptions(array &$map, array $options) {
    ...
    $map['settings']['booklet_markercluster'] = $options['booklet_markercluster'] ?? $default_settings['booklet_markercluster'];
  ...
  protected function setTooltipElement(array &$element, array $settings, array $view_fields = []) {
    $default_settings = $this::getDefaultSettings();
    $element['booklet_tooltip'] = [
      '#type' => 'fieldset',
      '#title' => $this->t('booklet Tooltip'),
    ];

    if (isset($this->fieldDefinition)) {
      $booklet_tooltip_visibility = [
        'invisible' => [
          'select[name="fields[' . $this->fieldDefinition->getName() . '][settings_edit_form][settings][booklet_tooltip][value]"]' => ['value' => ''],
        ],
      ];
    }
    else {
      $booklet_tooltip_visibility = [
        'invisible' => [
          'select[name="style_options[booklet_tooltip][value]"]' => ['value' => ''],
        ],
      ];
    }

    $tooltip_description = $this->t('Use this to insert a @booklet_tooltip (Feature by Feature).', [
      '@booklet_tooltip' => $this->link->generate("booklet Tooltip", Url::fromUri("https://leafletjs.com/reference.html#tooltip", ['attributes' => ['target' => 'blank']])),
    ]);

    if (isset($this->fieldDefinition)) {
      $element['booklet_tooltip']['value'] = [
        '#type' => 'textarea',
        '#title' => $this->t('Tooltip Source'),
        '#rows' => 2,
        '#default_value' => $settings['booklet_tooltip']['value'] ?? $default_settings['booklet_tooltip']['value'],
    ...
    $element['booklet_tooltip']['options'] = [
      '#type' => 'textarea',
      '#rows' => 3,
      '#title' => $this->t('Tooltip Options'),
      '#description' => $this->t('An object literal of options, that comply with the booklet Tooltip object definition.<br>The syntax should respect the javascript object notation (json) format.<br>As suggested in the field placeholder, always use double quotes (") both for the indexes and the string values.<br>Supports <b>Replacement Patterns</b>.'),
      '#default_value' => $settings['booklet_tooltip']['options'] ?? $default_settings['booklet_tooltip']['options'],
      '#placeholder' => $default_settings['booklet_tooltip']['options'],
      '#element_validate' => [[get_class($this), 'jsonValidate']],
      '#states' => $booklet_tooltip_visibility,
    ];
  }
  ...
  protected function setPopupElement(array &$element, array $settings, array $view_fields = [], string $entity_type = NULL, array $view_mode_options = []) {
    $default_settings = $this::getDefaultSettings();
    $element['booklet_popup'] = [
      '#type' => 'fieldset',
      '#title' => $this->t('booklet Popup'),
    ];

    if (isset($this->fieldDefinition)) {
      $booklet_popup_selector = 'fields[' . $this->fieldDefinition->getName() . '][settings_edit_form][settings][booklet_popup][control]';

      // Define the Popup Control and Popup Content with backward
      // compatibility with booklet release < 2.x.
      $popup_control = !empty($settings['popup']) ? $settings['popup'] : ($settings['booklet_popup']['control'] ?? NULL);
      $popup_content = !empty($settings['popup_content']) ? $settings['popup_content'] : ($settings['booklet_popup']['content'] ?? NULL);

      $element['booklet_popup']['control'] = [
        '#title' => $this->t('Enable booklet Popup'),
        '#description' => $this->t('Enable a @booklet_popup that will appear on Marker click.', [
          '@booklet_popup' => $this->link->generate("booklet Popup", Url::fromUri("https://leafletjs.com/reference.html#tilelayer", ['attributes' => ['target' => 'blank']])),
        ]),
        '#type' => 'checkbox',
        '#default_value' => $popup_control ?? $default_settings['booklet_popup']['control'],
      ];

      $element['booklet_popup']['content'] = [
        '#type' => 'textarea',
        '#title' => $this->t('Popup content'),
        '#rows' => 2,
        '#description' => $this->t('Define the custom content for the booklet Popup. If empty the Content Title will be output.<br>Supports <b>Replacement Patterns</b>.'),
        '#default_value' => $popup_content ?? $default_settings['booklet_popup']['content'],
        '#states' => [
          'visible' => [
            'input[name="' . $booklet_popup_selector . '"]' => ['checked' => TRUE],
          ],
        ],
      ];

      $element['booklet_popup']['options'] = [
        '#type' => 'textarea',
        '#title' => $this->t('Popup Options'),
        '#rows' => 3,
        '#description' => $this->t('An object literal of options, that comply with the booklet Popup object definition.<br>The syntax should respect the javascript object notation (json) format.<br>As suggested in the field placeholder, always use double quotes (") both for the indexes and the string values.<br><u>Note: if omitted, the "offset" option will be set on top of the feature icon size.</u><br>Supports <b>Replacement Patterns</b>.'),
        '#default_value' => $settings['booklet_popup']['options'] ?? $default_settings['booklet_popup']['options'],
        '#placeholder' => $default_settings['booklet_popup']['options'],
        '#element_validate' => [[get_class($this), 'jsonValidate']],
        '#states' => [
          'visible' => [
            'input[name="' . $booklet_popup_selector . '"]' => ['checked' => TRUE],
      ...
      $booklet_popup_selector = 'style_options[booklet_popup][value]';
      ...
      // compatibility with booklet release < 2.x.
      $popup_source = !empty($settings['description_field']) ? $settings['description_field'] : ($settings['booklet_popup']['value'] ?? NULL);
      $popup_view_mode = !empty($settings['view_mode']) ? $settings['view_mode'] : ($settings['booklet_popup']['view_mode'] ?? NULL);

      $element['booklet_popup']['value'] = [
        '#type' => 'select',
        '#title' => $this->t('Popup Source'),
        '#options' => $popup_options,
        '#default_value' => $popup_source ?? $default_settings['booklet_popup']['value'],
        '#description' => $this->t("Enable and choose content of a @booklet_popup that will appear on Marker click.", [
          '@booklet_popup' => $this->link->generate("booklet Popup", Url::fromUri("https://leafletjs.com/reference.html#tilelayer", ['attributes' => ['target' => 'blank']])),
        ]),
      ];
      ...
      $element['booklet_popup']['view_mode'] = [
        '#type' => 'select',
        '#title' => $this->t('Popup Source View mode'),
        '#description' => $this->t('View mode the entity will be displayed in the booklet Popup.'),
        '#options' => $view_mode_options,
        '#default_value' => $popup_view_mode ?? $default_settings['booklet_popup']['view_mode'],
        '#states' => [
          'visible' => [
            ':input[name="' . $booklet_popup_selector . '"]' => [
      ...
      $element['booklet_popup']['options'] = [
        ...
        '#states' => [
          'invisible' => [
            'select[name="' . $booklet_popup_selector . '"]' => ['value' => ''],
          ],
        ],
      ];
    }
  ...
  protected function setMapMarkerclusterElement(array &$element, array $settings, array $view_fields = []) {
    ...
    $default_settings = $this::getDefaultSettings();
    $booklet_markercluster_submodule_warning = $this->t("<u>Note</u>: This functionality and settings are related to the booklet Markercluster submodule, present inside the booklet module itself.<br><u>(DON'T USE the external self standing booklet Markecluster module).</u>");

    $element['booklet_markercluster'] = [
      '#type' => 'fieldset',
      '#title' => $this->t('Marker Clustering'),
    ];

    if ($this->moduleHandler->moduleExists('booklet_markercluster')) {
      $element['booklet_markercluster']['control'] = [
        '#type' => 'checkbox',
        '#title' => $this->t('Enable @markeclusterer_api_link functionality.', [
          '@markeclusterer_api_link' => $this->link->generate($this->t('booklet Markercluster Js Library'), Url::fromUri('https://github.com/Leaflet/Leaflet.markercluster', [
            'absolute' => TRUE,
            'attributes' => ['target' => 'blank'],
          ])),
        ]),
        '#default_value' => $settings['booklet_markercluster']['control'] ?? $default_settings['booklet_markercluster']['control'],
        '#description' => $this->t("@booklet_markercluster_submodule_warning", [
          '@booklet_markercluster_submodule_warning' => $booklet_markercluster_submodule_warning,
        ]),
        '#return_value' => 1,
      ];
      if (isset($this->fieldDefinition)) {
        $booklet_markercluster_visibility = [
          'visible' => [
            ':input[name="fields[' . $this->fieldDefinition->getName() . '][settings_edit_form][settings][booklet_markercluster][control]"]' => ['checked' => TRUE],
          ],
        ];
      }
      else {
        $booklet_markercluster_visibility = [
          'visible' => [
            ':input[name="style_options[booklet_markercluster][control]"]' => ['checked' => TRUE],
          ],
        ];
      }
      ...
      if (!empty($view_fields)) {
        $element['booklet_markercluster']['excluded'] = [
          '#type' => 'select',
          '#title' => $this->t('Exclude flag'),
          '#options' => array_merge([0 => 'No'], $view_fields),
          '#default_value' => $settings['booklet_markercluster']['excluded'] ?? $default_settings['booklet_markercluster']['excluded'],
          '#description' => $this->t("Choose a View Field as option to dynamically exclude a booklet Feature from being Marker Clustered (not Empty/FALSE/NULL instances/values will be excluded)."),
          '#states' => $booklet_markercluster_visibility,
        ];
      }

      $element['booklet_markercluster']['options'] = [
        '#type' => 'textarea',
        '#rows' => 4,
        '#title' => $this->t('Marker Cluster Additional Options'),
        '#description' => $this->t('An object literal of options, that comply with the booklet Markercluster Js Library.<br>The syntax should respect the javascript object notation (json) format.<br>As suggested in the field placeholder, always use double quotes (") both for the indexes and the string values.'),
        '#default_value' => $settings['booklet_markercluster']['options'] ?? $default_settings['booklet_markercluster']['options'],
        '#placeholder' => $default_settings['booklet_markercluster']['options'],
        '#element_validate' => [[get_class($this), 'jsonValidate']],
        '#states' => $booklet_markercluster_visibility,
      ];

      $element['booklet_markercluster']['include_path'] = [
        '#type' => 'checkbox',
        '#title' => $this->t('Enable Markeclustering of Paths elements'),
        '#default_value' => $settings['booklet_markercluster']['include_path'] ?? $default_settings['booklet_markercluster']['include_path'],
        '#description' => $this->t("Check this options to extend Markerclustering to the booklet Map features extending the @path_class_link (Polygon, Polyline, Circle).", [
          '@path_class_link' => $this->link->generate($this->t('booklet Path class'), Url::fromUri('https://leafletjs.com/reference.html#path', [
            'absolute' => TRUE,
            'attributes' => ['target' => 'blank'],
          ])),
        ]),
      ...
      $element['booklet_markercluster']['markup'] = [
        '#markup' => $this->t("Enable the booklet Markecluster submodule to activate this functionality.<br>@booklet_markercluster_submodule_warning", [
          '@booklet_markercluster_submodule_warning' => $booklet_markercluster_submodule_warning,
        ]),
      ];
    }
  ...
  protected function setFullscreenElement(array &$element, array $settings) {
    ...
    $element['fullscreen']['control'] = [
      ...
      '#title' => $this->t('Enable the functionality of the @fullscreen_api_link.', [
        '@fullscreen_api_link' => $this->link->generate($this->t('booklet Fullscreen JS Library'), Url::fromUri('https://github.com/Leaflet/Leaflet.fullscreen', [
          'absolute' => TRUE,
          'attributes' => ['target' => 'blank'],
        ])),
      ]),
      // Eventually fallback to previous "fullscreen_control" settings,
      // for backword compatibility with booklet < 2.x.
    ...
    $element['fullscreen']['options'] = [
      ...
      '#description' => $this->t('An object literal of options, that comply with the booklet Fullscreen JS Library.<br>The syntax should respect the javascript object notation (json) format.<br>As suggested in the field placeholder, always use double quotes (") both for the indexes and the string values.'),
  ...
  protected function setResetMapViewControl(array &$element, array $settings) {
    ...
    $element['reset_map']['control'] = [
      ...
      '#title' => $this->t('Enable @reset_map_view_link functionality.', [
        '@reset_map_view_link' => $this->link->generate($this->t('Reset Map View booklet Plugin'), Url::fromUri('https://github.com/drustack/Leaflet.ResetView', [
          'absolute' => TRUE,
          'attributes' => ['target' => 'blank'],
        ])),
      ]),
      '#description' => $this->t('This enables a "Reset Map View" control to reset the Map to its initial center & zoom state<br><b><u>Warning: </u></b>Due to an issue in the booklet library (@see https://github.com/Leaflet/Leaflet/issues/6172) the Map Reset control doesn\'t work correctly in Fitting Bounds of coordinates having mixed positive and negative values of latitude &longitudes.<br>In this case the Map will be Reset to the default set Map Center.'),
    ...
    $element['reset_map']['options'] = [
      ...
      '#description' => $this->t('An object literal of options, that comply with the booklet.ResetView Plugin<br>The syntax should respect the javascript object notation (json) format.<br>As suggested in the field placeholder, always use double quotes (") both for the indexes and the string values.'),
  ...
  protected function setMapScaleControl(array &$element, array $settings) {
    ...
    $element['map_scale']['options'] = [
      ...
      '#description' => $this->t('An object literal of options, that comply with the booklet Map Scale Options.<br>The syntax should respect the javascript object notation (json) format.<br>As suggested in the field placeholder, always use double quotes (") both for the indexes and the string values.'),
  ...
  protected function setLocateControl(array &$element, array $settings) {
    ...
    $element['locate']['control'] = [
      ...
      '#title' => $this->t('Enable @locate_link functionality.', [
        '@locate_link' => $this->link->generate($this->t('Locate booklet Plugin'), Url::fromUri('https://github.com/domoritz/leaflet-locatecontrol', [
          'absolute' => TRUE,
          'attributes' => ['target' => 'blank'],
        ])),
      ]),
    ...
    $element['locate']['options'] = [
      ...
      '#description' => $this->t('An object literal of options, that comply with the Locate booklet Plugin<br>The syntax should respect the javascript object notation (json) format.<br>As suggested in the field placeholder, always use double quotes (") both for the indexes and the string values.'),
      '#default_value' => $settings['locate']['options'] ?? $default_settings['locate']['options'],
      '#placeholder' => $default_settings['locate']['options'],
      '#element_validate' => [[get_class($this), 'jsonValidate']],
      '#states' => $booklet_locate_visibility,
    ];

    // Define the Automatic Locate Settings
    // for backwardcompatibility with booklet release < 2.x.
    ...
    if ($this->getPluginId() === 'booklet_widget_default') {
  ...
  protected function setFeatureAdditionalPropertiesElement(array &$element, array $settings) {
    ...
    $element['feature_properties']['values'] = [
      ...
      '#rows' => 3,
      '#title' => $this->t('Values'),
      '#description' => $this->t('Add additional key/value(s) that will be added in the "properties" index for each booklet Map "feature" (in the drupalSettings js object)<br>The syntax should respect the javascript object notation (json) format.<br>As suggested in the field placeholder, always use double quotes (") both for the indexes and the string values.<br>This is as advanced functionality, useful to dynamically alter booklet Map and each feature representation/behaviour on the basis of its properties.<br>Supports <b>Replacement Patterns</b>.'),
  ...
  protected function setGeocoderMapControl(array &$element, array $settings) {
    ...
    if ($this->moduleHandler->moduleExists('geocoder') && class_exists('\Drupal\geocoder\Controller\GeocoderApiEnpoints')) {
      ...
      $element['geocoder']['control'] = [
        ...
        '#description' => $this->t('This will add a Geocoder control element to the booklet Map'),
      ...
      $element['geocoder'] = [
        '#markup' => $this->t('<strong>Note: </strong>it is possible to enable a <u>Geocoder controller on the booklet Map</u> throughout the @geocoder_module_link integration (version higher than 8.x-2.3 and 8.x-3.0-alpha2).', [
