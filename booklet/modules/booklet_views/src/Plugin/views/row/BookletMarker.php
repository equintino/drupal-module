...
namespace Drupal\booklet_views\Plugin\views\row;
...
/**
 * Plugin which formats a row as a booklet marker.
 *
 * @ViewsRow(
 *   id = "booklet_marker",
 *   title = @Translation("Booklet Marker"),
 *   help = @Translation("Display the row as a booklet marker."),
 *   display_types = {"booklet"},
 *   no_ui = TRUE
 * )
 */
class BookletMarker extends RowPluginBase implements ContainerFactoryPluginInterface {
  ...
  public function render($row) {
    ...
    $result = $this->bookletService->bookletProcessGeofield($geofield_value);

    // Convert the list of geo data points into a list of booklet markers.
    return $this->renderBookletMarkers($result, $row);
  }

  /**
   * Converts the given list of geo data points into a list of booklet markers.
   *
   * @param array $points
   *   A list of geofield points from
   *   {@link "\Drupal::service('booklet.service')->bookletProcessGeofield()"}.
   * @param \Drupal\views\ResultRow $row
   *   The views result row.
   *
   * @return array
   *   List of booklet markers.
   */
  protected function renderBookletMarkers(array $points, ResultRow $row) {
    ...
    foreach ($points as &$point) {
      ...
      $this->alterBookletMarker($point, $row);

      // Allow modules to adjust the marker.
      $this->moduleHandler->alter('booklet_views_feature', $point, $row, $this);
    }
    return $points;
  }

  /**
   * Chance for subclasses to adjust the booklet marker array.
   *
   ...
   *   The Result rows.
   */
  protected function alterBookletMarker(array &$point, ResultRow $row) {
  }
