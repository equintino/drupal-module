...
namespace Drupal\booklet_views\Plugin\views\style;
...
use Drupal\booklet\BookletService;
use Drupal\booklet\BookletSettingsElementsTrait;
use Drupal\booklet_views\Controller\BookletAjaxPopupController;
...
/**
 * Style plugin to render a View output as a Booklet map.
 *
 ...
 * @ViewsStyle(
 *   id = "booklet_map",
 *   title = @Translation("Booklet Map"),
 *   help = @Translation("Displays a View as a Booklet map."),
 *   display_types = {"normal"},
 *   theme = "booklet-map"
 * )
 */
class BookletMap extends StylePluginBase implements ContainerFactoryPluginInterface {

  use LoggerChannelTrait;
  use BookletSettingsElementsTrait;
   ...
   * Booklet service.
   *
   * @var \Drupal\booklet\BookletService
   */
  protected $bookletService;
  ...
  /**
   * Constructs a BookletMap style instance.
   *
   ...
   *   The module handler.
   * @param \Drupal\booklet\BookletService $booklet_service
   *   The Booklet service.
  ...
  public function __construct(
    ...
    BookletService $booklet_service,
    ...
    $this->bookletService = $booklet_service;
    ...
    if (!empty($options['entity_source']) && $options['entity_source'] != '__base_table') {
      ...
      if (($table = $data->get($handler->definition['base'])) && !empty($table['table']['entity type'])) {
        try {
          ...
          $this->getLogger('Booklet View')->warning($e->getMessage());
        }
      }
    }
    else {
      ...
      if (is_a($handler, '\Drupal\views\Plugin\views\field\EntityField')) {
        ...
        if (array_key_exists($handler->definition['field_name'], $field_storage_definitions)) {
          ...
          try {
            ...
            $this->getLogger('Booklet View')->warning('No available data sources. Error: ' . $e->getMessage());
          }
        }
      }
    }
   ...
   *
   * @param \Drupal\booklet_views\Plugin\views\style\BookletMap $view_style
   *   The BookletMap View style.
   ...
   */
  public static function getOverlaysOptions(BookletMap $view_style, string $grouping_field_value, bool $grouping_rendered_value = FALSE) {
    $overlays = [];
  ...
  public function buildOptionsForm(&$form, FormStateInterface $form_state) {
    ...
    // Customise for this Booklet View Style the "grouping" section.
    $form["grouping"] = [
      '#type' => 'details',
      '#title' => $this->t("Booklet Grouping"),
      0 => $form["grouping"][0],
    ];

    $form["grouping"][0]["field"]["#title"] = $this->t('Grouping field');
    $form["grouping"][0]["field"]["#description"] = $this->t("You may optionally specify a field by which to group the Booklet Map Features by Overlayers, whose visibility could be managed throughout the Booklet Map Layers Control.<br>Leave blank to not group");
    ...
    // as we don't support it in Booklet View style map, at the moment.
    ...
    if (!count($fields_geo_data)) {
      $form['error'] = [
        ...
        '#attributes' => [
          'class' => ['booklet-warning'],
        ],
      ];
      return;
    }

    $wrapper_id = 'booklet-map-views-style-options-form-wrapper';
    ...
    if (count($entity_sources) == 1) {
      ...
      $form['entity_source'] = [
        ...
        '#description' => new TranslatableMarkup('Select which Entity should be used as Booklet Mapping base Entity.<br><u>Leave as "View Base Entity" to rely on default Views behaviour, and don\'t specifically needed otherwise</u>.'),
    ...
    $form['name_field'] = [
      ...
      '#description' => $this->t('Choose the field which will appear as as Simple Tooltip on mouse over each Booklet feature.'),
    ...
    // Generate the Booklet Map Reset Control.
    $this->setResetMapViewControl($form, $this->options);

    // Generate the Booklet Map Scale Control.
    ...
    // Generate the Booklet Map weight/zIndex Form Element.
  ...
  public function render() {
    ...
    $booklet_map_style = !isset($this->options['booklet_map']) ? $this->options['map'] : $this->options['booklet_map'];
    $map = booklet_map_get_info($booklet_map_style);

    // Set Map additional map Settings.
    $this->setAdditionalMapOptions($map, $this->options);

    // Add a specific map id.
    $map['id'] = Html::getUniqueId("booklet_map_view_" . $this->view->id() . '_' . $this->view->current_display);

    // Define the list of geofields set as source of Booklet View geodata,
    ...
    if (!empty($geofield_names) && (!empty($this->view->result) || !$this->options['hide_empty_map'])) {
      ...
      foreach ($view_results_groups as $group_label => $view_results_group) {
        ...
        // Iterate on each geofields set as source of Booklet View geodata.
        foreach ($geofield_names as $geofield_name) {
          ...
          if (isset($this->view->field[$geofield_name])) {
            ...
            foreach ($view_results_group['rows'] as $id => $result) {
              ...
              // and the $map.
              $booklet_view_geofield_value_alter_context = [
                'booklet_map_style' => $booklet_map_style,
                'result' => $result,
                'booklet_view_style' => $this,
              ];
              $this->moduleHandler->alter('booklet_map_view_geofield_value', $geofield_value, $map, $booklet_view_geofield_value_alter_context);

              if (!empty($geofield_value)) {
                $features = $this->bookletService->bookletProcessGeofield($geofield_value);
                ...
                if (!empty($entity_id) && !empty($entity_type)) {
                  ...
                  if (!isset($map['geofield_cardinality']) && isset($entity)) {
                    try {
                      ...
                      // @see: https://www.drupal.org/project/booklet/issues/3048089
                  ...
                  $popup_source = !empty($this->options['description_field']) ? $this->options['description_field'] : ($this->options['booklet_popup']['value'] ?? '');
                  $popup_view_mode = !empty($this->options['view_mode']) ? $this->options['view_mode'] : $this->options['booklet_popup']['view_mode'];
                  ...
                  switch ($popup_source) {
                    ...
                    case '#rendered_entity_ajax':
                      ...
                      $url = Url::fromRoute('booklet_views.ajax_popup', $parameters);
                      $popup_content = sprintf('<div class="booklet-ajax-popup" data-booklet-ajax-popup="%s" %s></div>',
                        $url->toString(), BookletAjaxPopupController::getPopupIdentifierAttribute($entity_type, $entity_id, $this->options['booklet_popup']['view_mode'], $langcode));
                  ...
                  // from hook_booklet_map_info.
                  if (!empty($map['icon'])) {
                    $this->options['icon'] = $this->options['icon'] ?: [];

                    // Remove empty icon options so that they might be replaced
                    // by the ones set by the hook_booklet_map_info.
                    foreach ($this->options['icon'] as $k => $icon_option) {
                      if (empty($icon_option) || (is_array($icon_option) && $this->bookletService->multipleEmpty($icon_option))) {
                  ...
                  foreach ($features as &$feature) {
                    ...
                    if (!empty($popup_content)) {
                      ...
                      $feature['popup']['options'] = $this->options['booklet_popup'] ? $this->options['booklet_popup']['options'] : NULL;
                    }

                    // Attach tooltip data (value & options),
                    // if tooltip value is not empty.
                    if (!empty($this->options['booklet_tooltip']['value'])) {
                      $feature['tooltip'] = $this->options['booklet_tooltip'];
                      // Decode any entities because JS will encode them again,
                      // and we don't want double encoding.
                      $feature['tooltip']['value'] = !empty($this->options['booklet_tooltip']['value']) ? Html::decodeEntities(($this->rendered_fields[$result->index][$this->options['booklet_tooltip']['value']])) : '';

                      // Associate dynamic tooltip options (token based).
                      if (!empty($this->options['booklet_tooltip']['options'])) {
                        $feature['tooltip']['options'] = str_replace([
                          "\n",
                          "\r",
                        ], "", $this->viewsTokenReplace($this->options['booklet_tooltip']['options'], $tokens));
                      }
                    }
                    ...
                    if (!empty($this->options['booklet_popup']['options'])) {
                      // Associate dynamic tooltip options (token based).
                      $feature['popup']['options'] = str_replace([
                        "\n",
                        "\r",
                      ], "", $this->viewsTokenReplace($this->options['booklet_popup']['options'], $tokens));
                    }
                        ...
                        default:
                          ...
                          if (!empty($this->options['icon']['iconUrl'])) {
                            ...
                            if (!empty($feature['icon']['iconUrl'])) {
                              $feature['icon']['iconUrl'] = $this->bookletService->generateAbsoluteString($feature['icon']['iconUrl']);
                            }
                          }
                          if (!empty($this->options['icon']['shadowUrl'])) {
                            ...
                            if (!empty($feature['icon']['shadowUrl'])) {
                              $feature['icon']['shadowUrl'] = $this->bookletService->generateAbsoluteString($feature['icon']['shadowUrl']);
                            }
                          }

                          // Set Feature IconSize and ShadowSize to the IconUrl
                          // or ShadowUrl Image sizes (if empty or invalid).
                          $this->bookletService->setFeatureIconSizesIfEmptyOrInvalid($feature);
                    ...
                    if ($this->options['booklet_markercluster'] && $this->options['booklet_markercluster']['control'] && !empty($this->options['booklet_markercluster']['excluded'])) {
                      $feature['markercluster_excluded'] = !empty(str_replace([
                        "\n",
                        "\r",
                      ], "", strip_tags($this->rendered_fields[$result->index][$this->options['booklet_markercluster']['excluded']])));
                    }
                    ...
                    $this->moduleHandler->alter('booklet_views_feature', $feature, $result, $this->view->rowPlugin);
                  }
                }
        ...
        if (count($view_results_groups) > 1) {
          ...
          $this->moduleHandler->alter('booklet_views_features_group', $group, $this);
      ...
      $this->moduleHandler->alter('booklet_map_view_style', $js_settings, $this);

      $map_height = !empty($this->options['height']) ? $this->options['height'] . $this->options['height_unit'] : '';
      $element = $this->bookletService->bookletRenderMap($js_settings['map'], $js_settings['features'], $map_height);
  ...
  protected function defineOptions() {
    ...
    $booklet_map_default_settings = [];
    foreach (self::getDefaultSettings() as $k => $setting) {
      $booklet_map_default_settings[$k] = ['default' => $setting];
    }
    return $options + $booklet_map_default_settings;
  }
